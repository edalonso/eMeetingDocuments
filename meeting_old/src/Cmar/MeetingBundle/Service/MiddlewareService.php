...
class MiddlewareService
{
    ...
    public function updateUser($old_in, $new_in)
    {
        ...
        $errors = $this->validator->validate($new_user);
        if (count($errors) > 0) {
            throw new \LogicException("Validation Exception \"" . $errors[0]->getPropertyPath() . "\": " . $errors[0]->getMessageTemplate(), -1);
        } 
        ...
        try {
            ...
            $sal = $this->adoAdmin->principalUpdate($user->getLogin(), $user->getEmail(), $user->getPassword(), $user->getName(), $user->getSurname(), "Automatically updated user in meeting", $id_user);  
        } catch (\Exception $e) {
